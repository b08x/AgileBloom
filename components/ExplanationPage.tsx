...
const InfoCard: React.FC<{ icon: React.ReactNode; title: string; children: React.ReactNode }> = ({ icon, title, children }) => (
  <div className="bg-gray-800/40 backdrop-blur-sm p-6 rounded-lg border border-gray-700/50">
    <div className="flex items-center mb-3">
      <div className="p-2 bg-purple-600/20 rounded-full mr-4 text-purple-300">
        {icon}
      </div>
...
const FlowStep: React.FC<{ icon: React.ReactNode; title: string; description: string }> = ({ icon, title, description }) => (
    <div className="flex flex-col items-center text-center p-4 bg-gray-800/20 rounded-lg max-w-[200px] flex-1">
...
const FlowArrow: React.FC = () => (
    <div className="flex-shrink-0 self-center text-purple-500/70 mx-2 hidden md:block">
        <ArrowRight size={32} />
    </div>
);
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-purple-900 to-blue-900 text-gray-100 font-sans flex items-center justify-center p-4 sm:p-6 md:p-8">
      <div className="max-w-6xl w-full mx-auto animate-fadeIn space-y-12">
        ...
        <header className="text-center">
          <h1 className="text-4xl sm:text-5xl font-bold tracking-tight text-purple-300">
        ...
        <main className="space-y-12">
            <section id="workflow-visual" className="p-6 md:p-8 bg-gray-800/30 backdrop-blur-md rounded-xl border border-purple-500/30">
                <h2 className="text-2xl sm:text-3xl font-semibold text-center mb-8 text-purple-300">The Core Workflow</h2>
          ...
          <section id="features" className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <InfoCard icon={<BookOpen size={24} />} title="The User Story Backlog">
                <p>The <strong className="text-purple-300">'Stories'</strong> tab is your central product backlog. Stories are the heart of the agile process, capturing a requirement from a user's perspective.</p>
            ...
            <InfoCard icon={<ListChecks size={24} />} title="Task Breakdown">
                <p>A story is a high-level goal. To make it actionable, it must be broken down into <strong className="text-purple-300">Tasks</strong>. In the 'Stories' tab, simply click the <strong className="text-purple-300">'Break Down'</strong> button on any story.</p>
            ...
            <InfoCard icon={<Star size={24} />} title="Prioritization & Estimation">
                 <p>A healthy backlog is a prioritized one. On each story card, you can set a <strong className="text-purple-300">Priority</strong> (from Low to Critical) and assign <strong className="text-purple-300">Sprint Points</strong> to estimate effort.</p>
